      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            type="text"
            icon="el-icon-edit"
            @click="handleSmartDetect(scope.row)"
            v-hasPermi="['system:oss:download']"
          >智能识别
          </el-button>
          <el-button
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="文件名">
          ...
          <fileUpload :file-size="1024" :file-type="['mp4']" v-model="form.file" v-if="type === 2"/>
        </el-form-item>
      </el-form>
...
import {helemetDetect} from "@/api/samrtdetect/helmetDetect";
...
export default {
  ...
  methods: {
    ...
    handleSmartDetect(row) {

     helemetDetect(row.url).then(res => {
       if (res.msg == "ok"){
         this.$router.push({
           path: '/baseuser/smartdetect',
           query: {
             video_url: row.url,
             detect_video_url: res.data,
           }
         })
       }
     })
